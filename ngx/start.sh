...
if [ -n "$GET_TRACKER_SERVER" ]; then
    ...
    export TRACKER_SERVER=$GET_TRACKER_SERVER
fi
...
function fdfs_set () {
    ...
    if [ -n "$TRACKER_SERVER" ] ; then  
        ...
        sed -i 's|^store_path0=.*$|store_path0='"${FASTDFS_BASE_PATH}"'|g' /etc/fdfs/tracker.conf
        sed -i 's|^store_path0=.*$|store_path0='"${FASTDFS_BASE_PATH}"'|g' /etc/fdfs/client.conf
        sed -i 's|^base_path=.*$|base_path='"${FASTDFS_BASE_PATH}"'|g' /etc/fdfs/*.conf
